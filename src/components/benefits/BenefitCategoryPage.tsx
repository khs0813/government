...
const categoryContent: Record<BenefitCategory, {
  ...
  youth: {
    ...
    body: [
      ...
      "청년도약계좌처럼 금융상품 성격이 있는 제도는 소득 확인과 가입 가능 은행 일정이 중요하고, 청년 월세 지원은 주민등록 주소, 임대차계약서, 월세 납부 증빙이 중요합니다."
    ],
...
export function BenefitCategoryPage({ category }: { category: BenefitCategory }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-14 md:px-6">
      <JsonLd data={breadcrumbJsonLd([{ name: "홈", path: "/" }, { name: "지원금", path: "/benefits/" }, { name: content.title, path: `/benefits/${category === "low_income" ? "low-income" : category}/` }])} />
